...
この関数は `file_path` で指定されたファイルに、トラッキングデータ `id, time, x, y` を一行に書き出す。
...
この関数は `file_path` で指定されたファイルに、pub/sub 関係付きトラッキングデータ `id, time, x, y,` と各トピックを pub/sub するのかを一行に書き出す。
...
この関数は `file_path` で指定されたファイルに、エッジサーバに対するクライアントの割り当て情報付きトラッキングデータ `id, time, x, y` と各トピックをどのエッジサーバに対して pub/sub するのかを一行に書き出す。
